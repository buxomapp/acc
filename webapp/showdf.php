...
<div class="d4">
  ...
  <tbody>
    <tr>
      ...
      <td class="ta2">-------</td>
      <td class="ta2">-------</td>
      <td class="ta2">تاریخ</td>
      <td class="ta2">مبلغ سرمایه </td>
      <td class="ta2">نام</td>
    </tr>
  </tbody>
</table> <form name="upshow" action="showdf?key=<?php echo $_GET["key"];?>&show=<?php echo $_GET["show"];?>&up=160" method="POST" >
	  <?php
include 'data/db.php';
$user = $_GET["show"];
$sql = "SELECT id, s1, s2, s3, s4, s5, s6, s7, s8, s9 FROM sa";
...
if ($result->num_rows > 0) {
  ...
  <tbody>
    <tr>
      <td class="ta2"><button type="submit"  class="btn btn-primary">ویرایش</button>
            </td>
      <td class="ta2">-----------------------------</td>
      <td class="ta2"><input  type="number" name="ed2" placeholder="'.$row["s7"].'"></input></td>
      <td class="ta2"><input type="number" name="ed1" placeholder="'.$row["s6"].'"></input></td>
